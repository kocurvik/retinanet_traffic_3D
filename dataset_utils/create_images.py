...
def add_cross(image, center, color):
    thickness = 3
    size = 25
    image = cv2.rectangle(image, (int(center[0]) - thickness, int(center[1]) - size),
                          (int(center[0]) + thickness, int(center[1]) + size), color, thickness=-1)
    image = cv2.rectangle(image, (int(center[0]) - size, int(center[1]) - thickness),
                          (int(center[0]) + size, int(center[1]) + thickness), color, thickness=-1)
    return image
...
def base_image():
    ...
    image = cv2.line(image, VP2, intersection(l1, line((0, 0), (1, 0))), (0, 0, 255), thickness=12,
                     lineType=cv2.LINE_AA)
    image = cv2.line(image, VP2, intersection(l2, line((0, 0), (1, 0))), (0, 0, 255), thickness=12,
                     lineType=cv2.LINE_AA)

    image = cv2.line(image, VP3, intersection(l3, line((0, 0), (0, 1))), (255, 0, 0), thickness=12,
                     lineType=cv2.LINE_AA)
    image = cv2.line(image, VP3, intersection(l4, line((0, 0), (0, 1))), (255, 0, 0), thickness=12,
                     lineType=cv2.LINE_AA)
...
def generate_image():
    ...
    json_path = 'D:/Skola/PhD/data/2016-ITS-BrnoCompSpeed/results/{}/system_SochorCVIU_Edgelets_BBScale_Reg.json'.format(
        session)
